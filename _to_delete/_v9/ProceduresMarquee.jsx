 ...
 * o espaçamento mora dentro de cada item (`pr`), e não num `gap` do <ul>: com
 * `gap`, 18 itens têm 17 intervalos, a metade da trilha cai no meio de um deles
 * e sobra meio intervalo — na prática, um salto de 16px a cada volta. Com o
 * espaço embutido, cada item é uma unidade fechada e a trilha é exatamente
 * 18 unidades, metade = 9.
 *
...
function Item({ procedure, hidden = false }) {
  return (
    <li className="flex shrink-0 items-center gap-6 pr-6 sm:gap-8 sm:pr-8" aria-hidden={hidden || undefined}>
      <Link
