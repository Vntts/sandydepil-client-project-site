...
const tema = {
  ...
  escuro: {
    ...
    veuDesktop: 'bg-gradient-to-r from-ink/85 via-ink/55 via-55% to-ink/20',
  },
}
      ...
      <div className="container-luxe relative z-10 pb-14 pt-28 md:py-24 lg:py-28">
        <div className="max-w-3xl">
          <motion.div
            ...
            className="mb-5 flex flex-wrap items-center gap-3 lg:mb-7"
          >
            {/* Os selos têm fundo branco próprio, então funcionam nos dois tons */}
          ...
          <div className="mt-5 grid lg:mt-7">
            {slides.map((s) => (
              <p key={`p-${s.id}`} className={`invisible [grid-area:1/1] ${classeTexto}`}>
                {s.texto}
              </p>
            ))}
            <p className={`[grid-area:1/1] transition-colors duration-700 ${classeTexto} ${t.texto}`}>
              <AnimatePresence mode="wait">
                <motion.span
                  key={slide.id}
          ...
          <motion.div
            ...
            className={`mt-6 flex items-center gap-2 text-[12px] transition-colors duration-700 sm:hidden ${t.nota}`}
          >
            <span className="flex gap-0.5">
              {Array.from({ length: 5 }).map((_, i) => (
